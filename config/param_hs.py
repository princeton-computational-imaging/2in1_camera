...
wvls = np.arange(400,701,10) *1e-9 # 31 HS wavelength np.array([400,550,700])*1e-9 #
...
slit_width_pxl = 8
DOE_phase_init = (sin_grating(int(2*slit_width_pxl), 90, 0, R)[None,None])*np.pi

depth = None

QE_weight = torch.load('utils/QE_weight.pt', map_location='cpu').detach()